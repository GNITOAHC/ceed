...
- [x] The Student loads in fp16 on a single V100 and produces coherent answers to DocVQA questions — verified before the remaining criteria are attempted _(verified 2026-07-25 on a V100 via `test_student_loads_in_fp16_and_answers_coherently`: finite logits and coherent answers on both the text and the vision/document path)_
- [x] If fp16 proves numerically unstable, the finding is recorded with evidence and the fallback path (fp32 for extraction) is costed before proceeding _(finding recorded in ADR-0006 Consequences: fp16 is stable for the Student, so the fp32 fallback is not needed for B0; the ~2× fp32 fallback remains costed for teacher extraction, confirmed at first extraction)_
